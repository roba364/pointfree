...
//  
//
//  Created by Sergey Borovkov on 04.01.2023.
//
    ...
    struct Event {
        ...
        static func tappedRepo(_ repo: GitHub.Repo) -> Event {
            return Event(
                ...
                properties: [
                    "repo_name": repo.name,
                    "build": Current.bundleVersionKey.bundleVersion,
                    "release": Current.bundleVersionKey.bundleShortVersion,
...
public struct Environment {
    ...
    public var bundleVersionKey: BundleVersionKey = .dev
}
...
public struct BundleVersionKey {
    public var bundleVersion: String
    public var bundleShortVersion: String
}

extension BundleVersionKey {
    static let dev = Self.init(bundleVersion: Bundle.main.object(forInfoDictionaryKey: "CFBundleVersion") as? String ?? "Unknown",
                               bundleShortVersion: Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String ?? "Unknown")
}

extension BundleVersionKey {
    static let mock = Self.init(bundleVersion: "mock", bundleShortVersion: "mock")
}

extension Environment {
    static let mock = Self.init(analytics: .mock,
                                date: { Date(timeIntervalSinceReferenceDate: 557152051) },
                                gitHub: .mock,
                                bundleVersionKey: .mock)
}
...
//}
extension GitHub {
    static let mock = Self.init(fetchRepos: { callback in
        callback(
            .success([
                GitHub.Repo(archived: false,
                            description: "Blob's blog",
                            htmlUrl: URL(string: "https://www.pointfree.co")!,
                            name: "Bloblog",
                            pushedAt: Date(timeIntervalSinceReferenceDate: 54700000))
            ])
        )
    })
}
...
//    }
//}
public let reposViewController = ReposViewController()
